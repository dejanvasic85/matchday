...
| `ladders` (table)                                              | ❌              | ✅ `team_hash_id`   | club_code, club_name, club_logo (per team row)      |
...
- **`team_hash_id`** — present on every table and fixture row. The only rock-solid stable id on
  the crawl path.
- **`club_code`** (e.g. `"OAKC"`) — present on every table row, but **only in table data**; the
...
| Table clubs → `list/clubs` **by logo**  | **100% matched** (218/218), 0 name fallback needed |
...
- The 9 referees + `FV Registrations` **never appear on any table** (checked across ~85 leagues).
- **`FV Academy` is the exception** — it fields teams and _does_ appear on tables.

So the distinguishing signal is **not** name or shared logo — it's **table presence**. A real club
appears on a table (has a `club_code`); an admin entity never does. This is why clubs must be
discovered from tables, not enumerated from `list/clubs` (which would ingest the admin junk and,
...
4. **The table row already supplies the team→club association** Dribl otherwise hides: each
...
- Table rows embed full `recent_matches`/`upcoming_matches` (scores, half-time, penalties, round,
  status) — a fixtures data source for the deep crawl to consider.

## Open (not blocking)

- Is `club_code` stable **across seasons** (only 2026 sampled)?
- Do no-table competitions (MiniRoos) still yield `club_code` via fixtures?
